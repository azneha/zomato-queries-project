...
/*4.what is the most purchased item on the manu and how many times it was purchased by the customers?*/
select userid,count(product_id) cnt from sales where product_id=
(select top 1 product_id cnt from sales group by product_id order by count(product_id) desc) 
group by userid

/*5.which item was purchased first by the customer after they become a member?*/
select * from
...
select userid , count(created_date) order_purchased,sum(price) ttlamntspend from 
(select c.*,d.price from
(select a.userid,a.created_date,a.product_id,b.gold_signup_date from sales a inner join goldusers_signup b on a.userid = b.userid 
and created_date<=gold_signup_date)c inner join product d on c.product_id=d.product_id)e
group by userid;

/*8.if buying each product generates points for eg 5rs=2 zomato point and each product have different 
purchasing points fr eg p1 5rs=1 zomato point,for p2 10rs = 5 zp and p3 5rs=1 zp
.calculate the points collected by each customer and for which product most points have been given till now*/ 

select userid,sum(ttlpoints) total_points_earned from
(select e.*,amt/points ttlpoints from
...
(select product_id,sum(ttlpoints) total_points_earned from
(select e.*,amt/points ttlpoints from
(select d.*,case when product_id=1 then 5 when product_id= 2 then 2 when product_id=3 then 5
else 0 end as points from
(select c.userid,c.product_id,sum(price) amt from 
...
/*9.In the first one year after a customer joins the gold program ( including the joining date )
irrespective of what the customer has purchased they earn 5 zomato points for every 10rs spent
, who earned more ? 1 or 3 ? what was their points earning in the first year?*/

select c.*,d.price*0.5 ttl_points from
(select a.userid,a.created_date,a.product_id,b.gold_signup_date from sales a inner join goldusers_signup b on a.userid = b.userid 
and created_date>=gold_signup_date and created_date<=DATEADD(year,1,gold_signup_date+365))c
inner join product d on c.product_id=d.product_id;

/*10.rnk all the transaction of the customers*/
select *,rank() over(partition by userid order by created_date) rnk from sales;

/*11.rank all the transaction for each member whenever they are a zomato gold member for 
every non gold member trans mark as na*/
